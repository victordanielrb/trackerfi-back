...
async function checkAlerts() {
  const client: MongoClient = mongo();
  try {
    ...
    while (await usersCursor.hasNext()) {
      const user: any = await usersCursor.next();
      if (!user || !user.alerts || user.alerts.length === 0) continue;

      for (let i = 0; i < user.alerts.length; i++) {
        const alert = user.alerts[i];
        if (!alert || !alert.token) continue;
        ...
        try {
          ...
          if (alert.token.address) {
            ...
            if (tokenDoc && typeof tokenDoc.price === 'number') currentPrice = tokenDoc.price;
          }
          ...
          if (currentPrice === null && user.wallets && Array.isArray(user.wallets)) {
            for (const w of user.wallets) {
              ...
              const found = (w.tokens || []).find((t: any) => (t.address && alert.token.address && t.address.toLowerCase() === alert.token.address.toLowerCase()) || (t.symbol && alert.token.symbol && t.symbol.toLowerCase() === alert.token.symbol.toLowerCase()));
          ...
          if (currentPrice === null) continue; // unable to resolve price

          const threshold = Number(alert.price_threshold);
          let triggered = false;
          if (alert.alert_type === 'price_above' && currentPrice > threshold) triggered = true;
          if (alert.alert_type === 'price_below' && currentPrice < threshold) triggered = true;

          if (triggered) {
            // Update the specific alert element with last_triggered timestamp
            const updateResult = await db.collection('users').updateOne(
              { _id: new ObjectId(user._id), 'alerts.token.address': alert.token.address, 'alerts.price_threshold': alert.price_threshold, 'alerts.alert_type': alert.alert_type },
              { $set: { 'alerts.$.last_triggered': nowIso } }
            );
            ...
            try {
              ws.notifyUser(user._id, { type: 'alert_triggered', userId: user._id, token: alert.token, price: currentPrice, triggered_at: nowIso });
            } catch (e) {
...
export default { startAlertsPolling };
