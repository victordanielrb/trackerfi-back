...
export const COINGECKO_PLATFORM_MAP: { [key: string]: string } = {
  // Major EVM chains
  'ethereum': 'ethereum',
...
export const NATIVE_TOKEN_ADDRESS_MAP: { [chain: string]: { [nativeAddress: string]: string } } = {
  'arbitrum': {
 ...
 */
export function getCoinGeckoTokenAddress(address: string, chain: string): string {
  const normalizedChain = chain.toLowerCase().trim();
  const normalizedAddress = address.toLowerCase().trim();
  
  if (NATIVE_TOKEN_ADDRESS_MAP[normalizedChain]?.[normalizedAddress]) {
    const mappedAddress = NATIVE_TOKEN_ADDRESS_MAP[normalizedChain][normalizedAddress];
 ...
 */
export function getCoinGeckoPlatformId(chainName: string): string | null {
  const normalizedChain = chainName.toLowerCase().trim();
  return COINGECKO_PLATFORM_MAP[normalizedChain] || null;
}
...
export function getSupportedChains(): string[] {
  return Object.keys(COINGECKO_PLATFORM_MAP);
}
...
export function validateChains(chains: string[]): {
  validMappings: { [original: string]: string };
  unsupportedChains: string[];
} {
  const validMappings: { [original: string]: string } = {};
