...
const OHLC_TIMEFRAMES = [1, 7, 14, 30, 90, 180, 365] as const;
 ...
 */
export async function getTokenTradingData(id: string): Promise<TokenTradingData | null> {
 ...
 */
async function fetchTradingDataFromAPI(id: string): Promise<TokenTradingData | null> {
	...
	const safeGet = (url: string) => axios.get(url, axiosConfig).catch(() => ({ data: null }));
	...
	const [
		...
		// OHLC data
		ohlc1dRes, ohlc7dRes, ohlc14dRes, ohlc30dRes, ohlc90dRes, ohlc180dRes, ohlc365dRes
	] = await Promise.all([
		axios.get(detailUrl, axiosConfig),
		// Market charts
	...
	const safeArray = (data: any) => (Array.isArray(data) ? data : undefined);

	const data: TokenTradingData = {
		id: detail.id,
...
export default getTokenTradingData;
