...
const coingeckoMapping_1 = require("./coingeckoMapping");
// Fallback function to get prices by symbol
async function fallbackPricesBySymbol(tokens) {
    try {
...
async function getTokenPrice(filterToken, client) {
    ...
    try {
        for (const [chain, tokens] of Object.entries(filterToken)) {
            ...
            const platformId = (0, coingeckoMapping_1.getCoinGeckoPlatformId)(chain);
            if (!platformId) {
            ...
            const mappedTokens = tokens.map(token => ({
                ...token,
                coingeckoAddress: (0, coingeckoMapping_1.getCoinGeckoTokenAddress)(token.address, chain)
            }));
