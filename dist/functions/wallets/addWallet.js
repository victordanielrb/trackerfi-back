...
const verifyWalletOwnership = async (address, blockchain, userId) => {
...
const addWallet = async (user_id, blockchain, wallet_address) => {
    ...
    try {
        ...
        if (!Object.values(general_1.Blockchain).includes(blockchain)) {
        ...
        const authUser = await database.collection("login_users").findOne({ _id: new mongodb_1.ObjectId(user_id) });
        if (!authUser) {
        ...
        let user = await database.collection("users").findOne({ _id: new mongodb_1.ObjectId(user_id) });
        if (!user) {
        ...
        const addressValidation = await validateWalletAddress(wallet_address, blockchain);
        if (!addressValidation.valid) {
            return { status: 400, message: { error: addressValidation.reason || "Invalid wallet address" } };
        }
        // Verify wallet ownership (placeholder)
        const ownershipVerified = await verifyWalletOwnership(wallet_address, blockchain, user_id);
        ...
        if (userHasChain) {
            ...
            const updateResult = await database.collection("users").updateOne({ _id: new mongodb_1.ObjectId(user_id), "wallets.chain": blockchain }, {
                $set: {
        ...
        const pushResult = await database.collection("users").updateOne({ _id: new mongodb_1.ObjectId(user_id) }, { $push: { wallets: walletObj } });
