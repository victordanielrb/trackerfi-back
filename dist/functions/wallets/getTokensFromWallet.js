...
exports.default = getTokensFromWallet;
...
async function getTokensFromWallet(wallet, client) {
    ...
    if (shouldCloseClient) {
        try {
            ...
            console.warn('Mongo connect failed:', e);
        }
    }
    ...
    try {
        ...
        const totalValue = userTokens.reduce((s, tk) => s + (parseFloat(tk.value) || 0), 0);
        const totalChange = userTokens.reduce((s, tk) => s + ((parseFloat(tk.value) || 0) * (parseFloat(tk.price_change_24h) || 0)), 0);
        ...
        const updateRes = await usersColl.updateOne({ "wallets.address": wallet }, {
            $set: {
        ...
        if (updateRes.matchedCount === 0) {
            await usersColl.updateOne({ "wallets.address": wallet }, {
                $setOnInsert: {
                    name: null,
                    email: null,
                    wallets: [
                        {
                            address: wallet,
                            chain: tokens[0]?.chain ? tokens[0].chain : 'UNKNOWN',
